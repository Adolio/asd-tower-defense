...
public class FSM
{
	 ...
	 * 
	 * @param client
	 *            La référence vers le Client
	 * @param tcp
	 *            La référence vers le TCPControlThread qui appelle la FSM
	 */
	...
	/**
	 * Loggue le message vers l'objet Logger et vers le client, s'il existe...
	 * 
	 * @param message
	 *            Le message à logguer
	 */
	...
	/**
	 * Méthode principale de la classe : la FSM en elle même. Techniquemene une
	 * boucle while infinie avec un case sur la variable state pour déterminer
	 * l'état courant. Implémentation la plus commune pour ce genre de pattern.
	 * 
	 * @throws IOException
	 *             En cas IOException
	 */
	private void finiteStateMachine() throws IOException
	{
		while (true)
		{
			try
			{
				log("*** Next Stage : " + getCurrentStateName()
						+ " ****");
				switch (state)
				{
				/*
				...
				case STATE_ERROR:
					...
					break;
				/*
				 * Tier 0 : Wait for incoming connection Client : $GAMENAME
				 * "client" $VERSION ToClient : $GAMENAME "server" $VERSION
				 * ToClient : Game :$WIDTH $HEIGHT $STATE => Go State1 => $STATE
				 * = {open,full,closed}
				 */
				case STATE_INIT:
					// Send a question to the server
					tcp.send("CaveShooter client version "
							+ client.getVersion());
					...
					if (!serverVersion.equals(client.getVersion()))
					{
						...
						log("Bad protocol version : Server is \""
								+ serverVersion + "\" and client \""
								+ client.getVersion() + "\"");
				...
				/*
				 * Tier 1 : Wait for a player's name Client : Player :
				 * $PLAYERNAME ToClient : if used => Player : name in use => Go
				 * State1 else => Player : Ok, id $ID => Go State2
				 */
				case STATE_REGISTER:
				...
				/*
				 * Tier 2 : Wait for UDP Port Client : Port $UDP ToClient : Port
				 * $UDP => Go State3
				 */
				case STATE_PORT:
					log("Client active : " + activePlayer);
					tcp.send("Port: " + client.getUdpPort(activePlayer));
					udpPortServer = Integer
							.parseInt(tcp.get().split("Port:")[1].replace(" ",
									""));
				 ...
				 * : $MSG ToClient : Join : $ID $NAME Part : $ID Score : $ID
				 * $SCORE Msg : $MSG Msg : $ID $MSG Announce : $MSG Error : $MSG
				 */
				case STATE_UPDATE:
					log("Wait for TCP messages and UDP connexion...");
					// Wait for TCP Messages from the Server
					while (true)
						parseUpdateMessages(tcp.get());
	...
	public String getNext(String message, String expr)
			throws IllegalStateException
	{
		return split(message, expr)[1];
	}

	private String[] split(String message, String pattern)
			throws IllegalStateException
	{
	...
	private void parseUpdateMessages(String message) throws ProtocolException
	{
		...
		if (message.equals("null"))
			throw new ProtocolException(
					"During the parsing : Null message from the server");
		try
		{
			...
			} else if (cmd.equals("Announce"))
			{
				...
				throw new ProtocolException(
						"Protocole Error : No matching found");
		...
		} catch (IllegalStateException e)
		{
			...
			e.printStackTrace();
			throw new ProtocolException(
					"Protocole Error : IllegalStateException");
		}
	}
}
