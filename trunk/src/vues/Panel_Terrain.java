 ...
 * 
 * Ce panel affiche la zone de jeu en elle-même. Celle-ci affichera les tours
 * avec les créatures et gèrera le positionnement des tours et la selection des
 * tours.
 * 
 ...
 */
public class Panel_Terrain extends JPanel implements Runnable, MouseListener,
		MouseMotionListener, KeyListener
{
	...
	private static final BasicStroke TRAIT_TILLE = new BasicStroke(1.0f,
			BasicStroke.CAP_ROUND, BasicStroke.JOIN_MITER, 10.0F, DASHES, 0.F);
	 ...
	 * d'interaction. Le joueur pourra alors améliorer ou detruire la tour.
	 */
	 ...
	 * 
	 * Cette methode affiche la scene du jeu. Elle recupere differents elements
	 * du jeu et les affiches.
	 * 
	 * @param g
	 *            Le graphics du panel pour dessin
	 */
	public void paint(Graphics g)
	{
		...
		if (afficherMaillage)
		{
			...
			if (arcsActifs != null)
				for (Line2D arc : arcsActifs)
				{
					...
					g2.drawLine((int) arc.getX1(), (int) arc.getY1(), (int) arc
							.getX2(), (int) arc.getY2());
				}
			/*
			 ...
			 * for(Noeud n : noeuds) {
			 * 
			 * if(n.isActif()) g2.setColor(Color.GREEN); else
		...
		while (iCreatures.hasNext())
		{
			...
			int positionXBarre = (int) (creature.getX() - creature.getWidth()
					/ 2 - (largeurBarre - creature.getWidth()) / 2);
			int positionYBarre = (int) (creature.getY() + creature.getHeight()
					/ 2 + 2);

			// affichage des barres de vie
			g2.setColor(COULEUR_CONTENEUR_SANTE);
			g2.fillRect(positionXBarre, positionYBarre, largeurBarre,
					HAUTEUR_BARRE_VIE);

			g2.setColor(COULEUR_SANTE);
			g2.fillRect(positionXBarre + 1, positionYBarre + 1, (int) (creature
					.getSante()
					* largeurBarre / creature.getSanteMax()) - 2,
					HAUTEUR_BARRE_VIE - 2);
		...
		if (tourAAjouter != null && sourisSurTerrain)
		{
			...
			if (!jeu.laTourPeutEtrePosee(tourAAjouter))
			{
				...
				g2.drawLine(sourisCaseX, (int) (sourisCaseY + tourAAjouter
						.getHeight()), (int) (sourisCaseX + tourAAjouter
						.getWidth()), sourisCaseY);

				g2.drawRect(sourisCaseX, sourisCaseY, (int) (tourAAjouter
						.getWidth()), (int) (tourAAjouter.getHeight()));
			} else
			...
			final boolean avecPortee)
	{
		...
		for (int i = 0; i < tour.getNiveau() - 1; i++)
		{
			...
			g2.fillRect(tour.getXi() + 4 * i + 1, (int) (tour.getYi()
					+ tour.getHeight() - 4), 3, 3);
			g2.setColor(COULEUR_NIVEAU);
			g2.fillRect(tour.getXi() + 4 * i + 2, (int) (tour.getYi()
					+ tour.getHeight() - 3), 1, 1);
		}
		// dessin de la portee
		if (avecPortee)
			dessinerPortee(tour, g2);
	...
	{
		g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,
				ALPHA_PERIMETRE_PORTEE));
		// g2.setColor(tour.getCouleurDeFond());
		g2.setColor(COULEUR_RAYON_PORTEE);
		g2
				.drawOval((int) (tour.getXi() - tour.getRayonPortee() + tour
						.getWidth() / 2), (int) (tour.getYi()
						- tour.getRayonPortee() + tour.getHeight() / 2),
						(int) tour.getRayonPortee() * 2, (int) tour
								.getRayonPortee() * 2);

		g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,
				ALPHA_SURFACE_PORTEE));
		g2.setColor(COULEUR_RAYON_PORTEE);
		g2
				.fillOval((int) (tour.getXi() - tour.getRayonPortee() + tour
						.getWidth() / 2), (int) (tour.getYi()
						- tour.getRayonPortee() + tour.getHeight() / 2),
						(int) tour.getRayonPortee() * 2, (int) tour
								.getRayonPortee() * 2);
	 ...
	 * 
	 * L'implémentation de Runnable nous force à définir cette méthode. Celle-ci
	 * sera appelée lors du démarrage du thread.
	 * 
	 * @see Runnable
	 */
	public void run()
	...
	public void mousePressed(MouseEvent me)
	{
		if (me.getButton() == MouseEvent.BUTTON1)
		{
			...
			for (Tour tour : jeu.getTours())
				// pour chaque tour...
				if (tour.intersects(sourisX, sourisY, 1, 1)) // la souris est
																// dedans ?
				{
