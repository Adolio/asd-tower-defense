...
public class TcpControlThread extends Thread {
    ...
    public TcpControlThread(String address, int port, Client client) throws IOException{
	try {
	    ...
	    Logger.err("Connection refused to "+serverAddress+":"+tcpServerPort);
	    exit();
	}
    }
    ...
    /**
     * This method is executed by the Client object when the user wants to send a message.
     */
	...
	log("[Send] \"" + message + "\"");
	if (serverSocket == null) throw new IOException("The serverSocket is not running...");
	else serverSocket.getOutputStream().write(( message + "\n" ).getBytes());
    }
    ...
    */
    ...
    /**
     * Méthode personnelle... il existe une méthode de l'API, mais
     * il est plus pédagogique de la comprendre par soi même
     * @param socket
     * @return
     * @throws IOException
     */
	...
	do {
	    ...
	    if (in == -1) throw new IOException("EOF from the Socket");
	    // If end of line 
	    if (in == (int) ENDOFLINE) break;
	    // Put the char in the buffer
	    else buffer.append( (char) in );
	} while (true);
	...
	log("Connect to " + target.getHostAddress() + " at port " + tcpServerPort);
	serverSocket = new Socket();
	serverSocket.connect(new InetSocketAddress(target, tcpServerPort), connectionTimeoutMillis);
	// Log : OK
	log("Connection OK");
    }
