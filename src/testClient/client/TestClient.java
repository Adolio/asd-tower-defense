...
public class TestClient implements Runnable
{
	...
	private final static String NAME = "TestClient";
	...
	private static String ipAdresseServer = "localhost";
	...
	private TestClient()
	{
		log("*** Test Client ASD TD " + VERSION + " ***");
	...
	public void run()
	{
		try
		{
			tcpAgent = new TcpControlThread(ipAdresseServer, tcpPortServer,
					null);
			finiteStateMachine();
		} catch (IOException e)
		{
	...
	private void finiteStateMachine() throws Exception
	{
		while (!QUIT)
		{
			log("*** Next Stage : \033[33m" + getCurrentStateName()
					+ "\033[00m ****");
			switch (state)
			{
			/*
			...
			case STATE_ERROR:
				...
				break;
			/*
			 * Tier 0 : Wait for incoming connection Client : $GAMENAME "client"
			 * $VERSION ToClient : $GAMENAME "server" $VERSION ToClient : Game
			 * :$WIDTH $HEIGHT $STATE => Go State1 => $STATE =
			 * {open,full,closed}
			 */
			case STATE_INIT:
				...
				{
					// Error, bad version
					error("Bad protocol version : Server is \"" + serverVersion
							+ "\" and client \"" + VERSION + "\"");
			...
			/*
			 * Tier 1 : Wait for a player's name Client : Player : $PLAYERNAME
			 * ToClient : if used => Player : name in use => Go State1 else =>
			 * Player : Ok, id $ID => Go State2
			 */
			case STATE_REGISTER:
				...
				{
					System.out.println("The name \"" + playerName
			...
			case STATE_PORT:
				...
				udpPortServer = Integer
						.parseInt(tcpAgent.get().split("Port:")[1].replace(" ",
								""));
				state = STATE_UPDATE;
